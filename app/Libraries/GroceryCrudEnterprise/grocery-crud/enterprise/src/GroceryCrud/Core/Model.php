...
ORDER BY KU.TABLE_NAME, KU.ORDINAL_POSITION;");
    ...
    public function removeMultiple($ids)
    {
        ...
        if ($this->_where !== null) {
            if(is_array($this->_where)) {
                $selectWhere = array_merge($selectWhere, $this->_where);
            } else {
                $selectWhere[] = $this->_where;
            }
        }

        $remove->where($selectWhere);
    ...
    function updateRelationManytoMany($fieldInfo, $data , $primaryKey)
    {
        ...
        $removeSqlStatement = $sql->delete($this->getTableNameWithSchemaForOperations($fieldInfo->junctionTable));
        ...
        $removeSqlStatement->where([$whereDelete]);
        $statement = $sql->prepareStatementForSqlObject($removeSqlStatement);
