...
class ExportState extends DatagridState {
    ...
    public function getPerPage($data = []) {
        // No matter of our data or default configurations, if we have set the perPage we will get the value from our object
        if ($this->perPage !== null) {
            return $this->perPage;
        }

        return ExportState::MAX_AMOUNT_OF_EXPORT;
    }
    ...
    public function getFinalData() {
        ...
        $visibleColumnsMinified = $stateParameters->visible_columns;
        // Filter data by visible columns if not empty
        if (!empty($visibleColumnsMinified)) {
            $visibleColumns = $this->getVisibleColumns($this->gCrud->getColumns(), $this->gCrud->getUnsetColumns(), $visibleColumnsMinified);
            $results = $this->filterDataByVisibleColumns($results, $visibleColumns);
            $columns = $this->filterColumnsByVisibleColumns($columns, $visibleColumns);
        }
    ...
    public function filterDataByVisibleColumns($data, $visibleColumns) {
        $finalData = [];
    ...
    public function filterColumnsByVisibleColumns($columns, $visibleColumns) {
        $finalColumns = [];
        foreach ($columns as $column) {
            if (isset($visibleColumns[$column->name])) {
                $finalColumns[] = $column;
            }
        }
        return $finalColumns;
